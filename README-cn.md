...
llm-test serve --host 0.0.0.0     # 监听所有接口（如 Tailscale 网络）
llm-test serve --reload           # 开发模式，代码修改自动重启
llm-test serve --test-mode        # 启用预设中转站快速选择
```
...
- **测试模式** — 使用 `--test-mode` 启动时，首页会显示预设中转站下拉选择器，自动填充 Base URL、API Key 和协议类型（来自 `config/presets.yaml`）。适合需要反复测试同一组中转站的场景，免去每次重复输入。
...
测试模式在首页添加预设中转站选择器，免去每次测试都要重新输入 URL 和 API Key。
...
预设下拉框会出现在测试表单顶部。选择预设后自动填充所有字段。下拉框中 API Key 仅显示后 4 位，完整密钥在选择预设时才传输。

`config/presets.yaml` 已加入 `.gitignore`，不会提交 API 密钥。不加 `--test-mode` 启动时，预设功能完全禁用（API 返回 404，界面隐藏选择器）。
...
config/
  ...
  presets.yaml.example    预设中转站模板（测试模式用）
